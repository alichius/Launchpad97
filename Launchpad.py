...
class Launchpad(ControlSurface):
    ...
    def _set_suppress_features(self, state):
        self._suppress_session_highlight = state # enable/disable the "Red box"
        self._suppress_send_midi = state # sounds like a good idea
    ...
    def build_midi_map(self, midi_map_handle):
        ...
        if self._selector!=None:
            if self._selector._main_mode_index==2 or self._selector._main_mode_index==1:
                mode = Settings.USER_MODES[2+ (self._selector._main_mode_index-1) * 3 + self._selector._sub_mode_list[self._selector._main_mode_index] ] 
